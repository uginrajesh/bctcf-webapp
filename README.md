...
**Send a newsletter — `apps-script/newsletter-send.gs` (automatic, Drive-folder driven, PDF-anchored):**
Each month, write the newsletter in Word locally, export a **PDF**, and upload that PDF (e.g. `June 2026 Newsletter.pdf`) to a dedicated Drive folder. A time-driven trigger picks up the newest PDF, emails every subscriber with the PDF attached and a short note pointing to it, then records it so it's never sent twice. The PDF is the newsletter (it keeps all its layout/images); the email body is the short `GENERIC_BODY`. You may also drop the source `.docx` in the folder for your own records — the script ignores everything that isn't a PDF, and archives a same-named `.docx` alongside the PDF. Setup: Script Properties `SHEET_ID` (subscribers sheet) + `NEWSLETTER_FOLDER_ID` (the Drive folder) + optional `ARCHIVE_FOLDER_ID` (a subfolder the sent PDF + matching `.docx` are moved into after sending), and a time-driven trigger on `checkAndSendNewsletter` (Hour/Day timer). Guardrails so an accidental run can't blast email: never sends the same PDF twice (`LAST_SENT_PDF_ID`), at least `MIN_DAYS_BETWEEN` (24) days between sends, and the PDF must be untouched for `MIN_SETTLE_MINUTES` (30) so a file you're still uploading won't go out mid-upload.

**Unsubscribe.** Each newsletter ends with a footer inviting the reader to reply to be removed. Replies land in `bctamilcatholicfamily@gmail.com`; a coordinator sets that person's **Status** to `unsubscribed` in the Subscribers sheet (column C) and `newsletter-send.gs` skips them thereafter. `apps-script/newsletter-unsubscribe.gs` can automate this - it scans the inbox for "unsubscribe" replies and flips the status - but it ships **disabled** (`ENABLED = false`, no trigger). To turn it on later: drop it in the same Apps Script project as the send script (it reuses `SHEET_ID`), set `ENABLED = true`, run `processUnsubscribes` once to grant the Gmail authorization, and add an hourly time-driven trigger. It only matches replies from the address the person subscribed with; anything else is left unread and logged for manual review. Full step-by-step enable guide: [`apps-script/NEWSLETTER-UNSUBSCRIBE.md`](apps-script/NEWSLETTER-UNSUBSCRIBE.md).
